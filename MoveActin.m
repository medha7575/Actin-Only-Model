    ...
    for i = 1: Ac_NodeCount
        ...
        for R= NewrowNo-Aura : NewrowNo+Aura   % search all around the node for myosin
            for C= NewcolNo-Aura:NewcolNo+Aura
                ...
                % % % if there is a wall node in the vicinity of the new
                % position, actin node is kept in the old position
				
                if Wa_NodeNo(round(R),round(C))>-1  %% check if node is a wall node
        ...
        %%%%%%%%%%%%%%%%%------------------ %%%%%%%%%%
        %%---Updating new actin node features----
        Ac_NodeNo(round(Ac_Node(NodeID,ROW)),round(Ac_Node(NodeID,COL)))= -1;
        %         Ac_Node=UpdateNode(NodeID, Ac_Node, NewrowNo, NewcolNo,Ac,NPF);
...
%                
                ...
                BRcolNo=round((BrPoint_col+Fnpf_c),2);  % branched actin position
                 ...
                 if Wa_NodeNo(round(BRrowNo),round(BRcolNo))> -1  %% just check for presence of wall on that point
                                    disp('cant branch');
                 else
